...
export default function ProList() {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/Models/Process/getProfessionals.php')
  ...
  return (
    <ul role="list" className="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800 rounded-t-lg">
      {people.map((person) => (
        <li key={person.id} className="flex justify-between gap-x-6 py-5 px-5">
          ...
          <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
            <p className="font-semibold">{person.proType}</p>
            {person.lastSeen ? (
